...
let createInfo = {
  help(commandStore, name) {
    if(!Array.isArray(commandStore)){return false;}
    let msg = [];
...
class Client extends utils.Event {
  ...
  parse({ body, headers }) {
    let out = this.route({ body, headers });
    let { status, result } = out;
    if (status === false) {
      return out;
    } else if (status === true) {
      let { cmdOption, type, message, value, data } = result;
      return {
        status,
        data: {
  ...
  async handleCallback(result, allCallback) {
    ...
    if (config.help===true && value === 'notification' && innerCmd === 'help' && focusApp !== null) {
      ...
      if(config.errorHelp===true){//auto trigger error help
  ...
  handle({ body, headers }, allCallback) {
    if (typeof allCallback === 'function') {
      return this.handleCompatibility({ body, headers }, allCallback);
    } else {
      return new Promise((resolve, reject) => {
  ...
  handleCompatibility({ body, headers }, allCallback) {
    if (typeof body !== 'object') {
      //only check body
      if (typeof allCallback === 'function') {
